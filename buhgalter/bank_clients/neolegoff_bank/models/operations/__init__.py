...
from bank_clients.neolegoff_bank.models.base import Amount, Location
...
class FieldsValues(BaseModel):
    ...
    merchant_display_name: str | None = Field(
        None, alias="merchantDisplayName")
    mcc: int | None
...
class OperationMinimal(BaseModel):
    ...
    is_inner: bool = Field(alias="isInner")
    loyalty_bonus_summary: Optional[LoyaltyBonusSummary] = Field(
        None, alias="loyaltyBonusSummary")
    has_statement: bool = Field(alias="hasStatement")
    id: str
    type: Literal["Debit", "Credit"]
    locations: list[Location]
    description: str
    status: Literal["OK", "WAITING", "FAILED"]
    ...
    def add_raw(cls, values: dict):
        ...
        if "debitingTime" in values:
            ...
            values["debited_at"] = datetime.fromtimestamp(
...
class OperationBase(OperationMinimal):
    ...
    sender_details: str | None = Field(None, alias="senderDetails")
